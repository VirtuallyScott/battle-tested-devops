...
###############################################################################
# Script: recon_matrix.sh
# Description: Enhanced recon script with DNS matrix, www lookup, and HTML table
###############################################################################
...
Performs a full recon of the given domain, including:
  - WHOIS on domain and resolved IP
  - NSLOOKUP for domain and www.domain
  - Ping, SPF, DKIM, DMARC, DNSSEC checks
  - MX record and SMTP connectivity test
  - Optional subdomain enumeration
  - Outputs HTML or JSON report (default is HTML)
EOF
    exit 0
}
...
[[ -z "$DOMAIN" ]] && { echo "[ERROR] Domain is empty"; exit 1; }
[[ "$OUTPUT" != "html" && "$OUTPUT" != "json" ]] && { echo "[ERROR] Invalid output format: $OUTPUT"; exit 1; }

log "Domain set to $DOMAIN"
log "Output format set to $OUTPUT"
...
IP_ADDR=$(echo "$NSLOOKUP_DOMAIN" | awk '/^Address: /{print $2}' | head -n 1)
[[ -z "$IP_ADDR" ]] && { echo "[ERROR] Could not resolve $DOMAIN"; IP_ADDR="N/A"; }

WHOIS_IP=$(whois "$IP_ADDR" 2>/dev/null | sed 's/"/\"/g' || echo "WHOIS failed")
...
while read -r priority mxhost; do
    ...
    MX_IPS+=("$mxhost ($ip)")
    SMTP_CHECK=$(echo | openssl s_client -connect ${mxhost}:25 -starttls smtp 2>/dev/null || echo "SMTP test failed")
    SMTP_RESULTS+=("$SMTP_CHECK")
...
log "Checking TLS/SSL for domain and www.domain..."
...
log "Saving HTML output to $OUTPUT_FILE"
    {
        echo "<html><head><style>"
        echo "body{font-family:sans-serif;background:#f4f4f4;padding:20px}"
        echo "h1{color:#00457c}pre{background:#fff;padding:10px;border:1px solid #ccc}"
        echo "table{background:#fff;border-collapse:collapse;margin-bottom:20px}"
        echo "td,th{border:1px solid #ccc;padding:8px}th{background:#00539C;color:#fff}"
        echo "summary{cursor:pointer;background:#00539C;color:#fff;padding:10px;border-radius:5px}"
        echo "</style></head><body><h1>Recon Report: $DOMAIN</h1>"

        echo "<table><tr><th>Domain</th><th>Registered</th><th>Expires</th><th>DNS Servers</th></tr>"
        echo "<tr><td>$DOMAIN</td><td>$REG_DATE</td><td>$EXP_DATE</td><td><ul>"; for ns in $(echo $NS_LIST | tr "," "\n"); do echo "<li>$ns</li>"; done; echo "</ul></td></tr></table>"

        echo "<details open><summary>IP Address</summary><pre>$IP_ADDR</pre></details>"
        ...
        echo "<details><summary>WHOIS IP</summary><pre>$WHOIS_IP</pre></details>"
...
echo "<details><summary>TLS/SSL - www.$DOMAIN</summary><pre>${SSL_WWW:0:1500}...</pre></details>"
    ...
    echo "[WARN] JSON output not yet supported in matrix mode."
fi

log "Recon complete: $OUTPUT_FILE"
